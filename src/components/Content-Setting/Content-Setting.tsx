import React from 'react';
import { Typography, Flex, Form, Input } from 'antd';
import { FormInstance } from 'antd/lib/form';

import 'quill/dist/quill.snow.css'
import ReactQuill from 'react-quill'

export interface FormProps {
    form: FormInstance<any>;
}

var modules = {
    toolbar: [
        [{ size: ["small", false, "large", "huge"] }],
        ["bold", "italic", "underline", "strike", "blockquote"],
        [{ list: "ordered" }, { list: "bullet" }],
        ["link", "image"],
        [
        { list: "ordered" },
        { list: "bullet" },
        { indent: "-1" },
        { indent: "+1" },
        { align: [] }
        ],
        [{ "color": ["#000000", "#e60000", "#ff9900", "#ffff00", "#008a00", "#0066cc", "#9933ff", "#ffffff", "#facccc", "#ffebcc", "#ffffcc", "#cce8cc", "#cce0f5", "#ebd6ff", "#bbbbbb", "#f06666", "#ffc266", "#ffff66", "#66b966", "#66a3e0", "#c285ff", "#888888", "#a10000", "#b26b00", "#b2b200", "#006100", "#0047b2", "#6b24b2", "#444444", "#5c0000", "#663d00", "#666600", "#003700", "#002966", "#3d1466", 'custom-color'] }],
    ]
};

var formats = [
    "header", "height", "bold", "italic",
    "underline", "strike", "blockquote",
    "list", "color", "bullet", "indent",
    "link", "image", "align", "size",
];
const ContentSetting: React.FC<FormProps> = ({ form }) => {
    const handleProcedureContentChange = (content) => {
        console.log("content---->", content);
    };
    return (
        <Flex gap="large">
            <Form
                name="form1"
                labelCol={{ flex: '110px' }}
                labelAlign="left"
                labelWrap
                wrapperCol={{ flex: 1 }}
                colon={false}
                form={form}
                style={{ background: '#f9f9f9', padding: 50 }}
                initialValues={{ items: [{}] }}
                >
                <Form.List name="items">
                    {
                        (fields, { add, remove }) => (
                        <>
                            {
                                fields.map((field, index) => (
                                    <Flex 
                                        key={field.key}
                                    >
                                        <div style={{ width: '600px' }}>
                                            <Form.Item label="Subject" name={[field.name, 'subject']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                            <Form.Item label="Content" name={[field.name, 'content']}  rules={[{ required: true }]}>
                                                <ReactQuill
                                                    theme="snow"
                                                    modules={modules}
                                                    formats={formats}
                                                    placeholder="write your content ...."
                                                    onChange={handleProcedureContentChange}
                                                    style={{ height: "220px" }}
                                                >
                                                </ReactQuill>
                                            </Form.Item>
                                        </div>
                                        <div style={{ width: '50%', paddingLeft: 40 }}>
                                            <Form.Item label="To Email" name={[field.name, 'toEmail']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                            <Form.Item label="From Name" name={[field.name, 'fromName']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                            <Form.Item label="From Email" name={[field.name, 'fromEmail']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                            <Form.Item label="Date Time" name={[field.name, 'fromDateTime']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                            <Form.Item label="CC" name={[field.name, 'cc']} rules={[{ required: true }]}>
                                                <Input />
                                            </Form.Item>
                                        </div>
                                    </Flex>
                                ))
                            }
                        </>
                    )
                }
                </Form.List>
                <div className="mt-32">
                    <Form.Item noStyle shouldUpdate>
                        {() => (
                        <Typography>
                            <pre>{JSON.stringify(form.getFieldsValue(), null, 2)}</pre>
                        </Typography>
                        )}
                    </Form.Item>
                </div>
            </Form>
        </Flex>
       
      )
};

export default ContentSetting;